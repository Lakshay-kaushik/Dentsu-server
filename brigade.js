...
class JobFactory {
  createSetupPipelineJob(e, project) {
    var setupPL = new Job("setup-pl", "globaldevopsreg11.azurecr.io/builder:latest")
    setupPL.storage.enabled = true
    ...
    setupPL.tasks = [
      ...
      `kubectl exec -t $MONGO_POD \
        ...
        -- mongo ${uniqueDatabase} -u root -p ${project.secrets.unittest_mongo_root_pass} \
        --authenticationDatabase admin \
        --eval "db.createUser({ user: '${project.secrets.unittest_mongo_user}', pwd: '${project.secrets.unittest_mongo_pass}', roles: [ { role: 'readWrite', db: '${uniqueDatabase}' } ] })"  || \
      echo DB already exists`,
      `echo -n ${uniqueDatabase} > /mnt/brigade/share/unitTestDatabase.txt`
    ]
  ...
  createBuildJob(e, project) {
    ...
    build.tasks = [
      ...
      "yarn build",
      `echo ${mongoPattern}`,
      `export MONGO_TEST_URL=${mongoPattern}`,
      "npm install -g pkg",
      "yarn packaging",
      // Lint
      "yarn lint",
...
events.on("push", (e, project) => {
  ...
  if (e.type == 'push') {
    if (jsonPayload.ref == "refs/heads/develop") {
      Group.runEach([
        jobFactory.createSetupPipelineJob(e, project),
        jobFactory.createBuildJob(e, project),
        jobFactory.dropUnitTestDBJob(e, project),
